...
- Заголовки # … ###### — название поста (#) и смысловые блоки (##). Уместны в посте любой длины, если тема того просит: решай по содержанию, а не по числу абзацев, и не ставь заголовок над каждым абзацем.
...
# Тон незаметно перетягивал и структуру: «экспертный» получал ##,
# «дружелюбный» — сплошной текст. Развязываем явно.
_TONE_SCOPE = ("Тон управляет лексикой и интонацией, но НЕ структурой: "
               "решение о заголовках, подзаголовках и выделениях принимай "
               "по правилам разметки, независимо от тона.")
...
def _tone_instruction(tone: str | None) -> str:
    ...
    if preset:
        return f"{preset}\n{_TONE_SCOPE}"
    return f"Тон и стиль поста: {tone[:200]}. {_TONE_SCOPE}"
...
AI_ACTIONS = {
    ...
    "generate": {
        "system": (
            ...
            "занимает пост в этом канале. Если пользователь сам задал объём "
            "(«коротко», "
            "«пара предложений», «подробно», «длинный», «со списком», "
            "конкретное число) — его просьба важнее этого правила.\n"
            "3. Разметка подбирается по содержанию, а не по формуле.\n"
            "Заголовки: сам решай, сколько их нужно посту. Тематическому "
            "или образовательному посту заголовок # к лицу при любой длине; "
            "и если в таком посте есть несколько смысловых блоков (принцип "
            "работы / риски / инструменты, тезис / примеры / вывод) — "
            "отбей их подзаголовками ##, даже когда пост среднего размера: "
            "читателю проще сканировать структуру. Заметке-размышлению "
            "заголовки не нужны и в пять абзацев. Ориентир: заголовок "
            "оправдан, когда помогает навигации или задаёт тему; не ставь "
            "## над каждым абзацем и не дублируй им первую фразу абзаца.\n"
            "Выделения: 0–2 на пост, подбирай под содержание и меняй приёмы "
            "от поста к посту, чтобы оформление не выглядело шаблоном. "
            "Палитра: **жирный** — критичный факт (дата, цена, число) или "
            "ударная фраза; _курсив_ — термин, ироничная оговорка, смысловой "
            "полутон; ~~зачёркнутый~~ — шутливая самоправка (было → стало). "
            "==Маркер== — редкий инструмент: максимум один и только в "
            "длинном посте, для итогового вывода-правила. ||Спойлер|| сам "
            "не добавляй — только когда пользователь прямо просит пост с "
            "загадкой или интригой. Чистый текст вовсе без выделений — "
            "тоже полноправный вариант: если ничего не просится, не "
            "выделяй, разметка ради разметки раздражает.\n"
            "Длинный пост (от 4 абзацев) без разметки — сплошная «простыня», "
            "её бросают читать: тут подзаголовки и один-два **жирных** "
            "акцента обязательны. Эмодзи — только если просят.\n"
...
def build_ai_messages(action: str, text: str, context: str | None = None,
    ...
    if tone_applies and tone_clean:
        ...
        content = (f"{content}\n\nТон ответа: {label} — тон влияет только "
                   f"на слова, а заголовки и выделения расставляй по "
                   f"правилам разметки, как без тона.")
    ...
    # Напоминание об оформлении — всегда (правило из глубины system-промпта
    # без него не срабатывает; с тоном это выяснилось раньше, без тона — то же)
    if action == "generate":
        content = (f"{content}\nОформление: тематическому или "
                   f"образовательному посту — заголовок # и подзаголовки ## "
                   f"его смысловых блоков; заметке, новости, размышлению — "
                   f"чистый текст без заголовков.")
