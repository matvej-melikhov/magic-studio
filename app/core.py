...
AI_FORMAT_GUIDE = """Доступная разметка Rich Markdown (Telegram Bot API 10.1):
...
AI_ACTIONS = {
    ...
    "format": {
        "system": (
            ...
            "1–2 ключевые фразы → **жирный**. Заголовок # — только если "
            "первая строка текста явно заголовочная.\n"
            "3. Не переусердствуй: лучше меньше разметки, чем больше. "
            "Если разметка не нужна — верни текст как есть.\n"
            f"4. {_ANSWER_RULE}"
        ),
        "examples": [
            (
...
def build_ai_messages(action: str, text: str, context: str | None = None) -> list[dict]:
    ...
    if context:
        content = f"Пост целиком, фрагмент помечен:\n\n{context}"
    else:
        content = text
