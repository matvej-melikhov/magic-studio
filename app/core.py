...
- ==выделение маркером== — редкий инструмент: правило или вывод, который читатель захочет сохранить; в большинстве случаев уместнее **жирный**.
...
AI_ACTIONS = {
    ...
    "generate": {
        "system": (
            ...
            "Короткий пост (до 3 абзацев): 0–2 выделения, подбирай их под "
            "содержание и меняй приёмы от поста к посту, чтобы оформление "
            "не выглядело шаблоном. Палитра: **жирный** — критичный факт "
            "(дата, цена, число) или ударная фраза; _курсив_ — термин, "
            "ироничная оговорка, смысловой полутон; ~~зачёркнутый~~ — "
            "шутливая самоправка (было → стало). ==Маркер== и ||спойлер|| "
            "в короткие посты сам не добавляй: спойлер уместен только когда "
            "пользователь прямо просит пост с загадкой или интригой. Чистый "
            "текст вовсе без выделений — тоже полноправный вариант: если "
            "ничего не просится, не выделяй, разметка ради разметки "
            "раздражает.\n"
            ...
            "фактах. ==Маркер== — редкий инструмент: максимум один, и только "
            "для итогового вывода-правила, которое читатель захочет "
            "сохранить; чаще обходись жирным. Эмодзи — только если просят.\n"
        ...
        "examples": [
            ...
            # дефолтная длина, фактов нет — другой приём: курсив на термине
            # (примеры показывают разные инструменты, а не один и тот же)
            (
                "пост-наблюдение о том, что лучшие идеи приходят на прогулке",
                "Заметил закономерность: стоит закрыть ноутбук и выйти "
                "пройтись, как задача, над которой бился весь день, "
                "решается сама. Пятнадцать минут без экрана — и в голове "
                "само раскладывается то, что не раскладывалось три часа "
                "за столом.\n\n"
                "Этому есть объяснение: мозгу нужен _режим рассеянного "
                "внимания_, чтобы связывать далёкие вещи между собой. За "
                "монитором мы удерживаем фокус, а инсайты рождаются как "
                "раз вне его. Так что застряли на задаче — не сидите над "
                "ней, идите гулять.",
            ),
            (
