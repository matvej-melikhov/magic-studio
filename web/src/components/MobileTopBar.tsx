...
/* Мобильная шапка (id и классы из editor.html). Диагностика вьюпорта —
   5 тапов, теперь по имени пользователя: логотип стал ссылкой на /about,
   и переход срабатывал бы с первого тапа. */
export default function MobileTopBar() {
  ...
  return (
    <div id="mtop">
      {/* относительный href сохраняет префикс стенда (/dev, /prod) */}
      <a id="mtopBrand" href="about" title="О сервисе">
        <div className="logo"><LogoMark size={15} /></div>
        <div className="name">Magic Studio</div>
      </a>
      <span className="spacer"></span>
      <span id="mtopUser" onClick={onDiagTaps}>{name}</span>
